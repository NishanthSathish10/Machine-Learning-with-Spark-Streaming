...
data_path = '/home/pes1ug19cs304/Desktop/Project/cluster_data.csv'
batch_no =1
batch_size=5000

def cluster(tweets,labels):
	global batch_no
	km_file = open(km_path, 'rb')
	km = pickle.load(km_file)
	y_pred = km.predict(tweets)
	...
	values = pca.fit_transform(tweets)
	pca1 = values[:,0]
	pca2 = values[:,1]
	pca3 = values[:,2]
	data_file = open(data_path, 'a')
	for i in range(5000):
		data_file.write(f'{batch_no},{pca1[i]},{pca2[i]},{pca3[i]},{y_pred[i]},{labels[i]}\n')
	data_file.close()
	#fig=plt.figure()
	#plt.ion()
	#plt.scatter(pca1, pca2, c=y_pred, cmap='rainbow')
	#plt.savefig(f"{batch_no}.png")
	#plt.ioff()
...
def process_rdd(rdd):
    ...
    if df is not None:
        ...
        labels = np.array( [ 0 if i==4 else 1 for i in labels])
        tweets = hv.fit_transform(tweets)
        print('Preprocessing Done')
        cluster(tweets,labels)
